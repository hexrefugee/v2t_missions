...
pub_img = rospy.Publisher('/camera1/rgb/image_raw', Image, queue_size = 10)
pub_img_segment = rospy.Publisher('/camera1/rgb/seg_raw', Image, queue_size = 10)
...
def main():
    ...
    try:
        ...
        camera_at1 = world.spawn_actor(
            blueprint=camera_bp,
            transform=carla.Transform(carla.Location(100,190,5)))
        actor_list.append(camera_at1)
        camera_at1.listen(lambda data: camera_at1_callback(data))

        # 创建 Semantic segmentation 
        camera_transform = carla.Transform(carla.Location(100,190,5))
        ...
        # 设置激光雷达的点
        lidar_location = carla.Location(100,190,5)
